-- Esquema Unificado para PharmaSync en Supabase (Versión Limpia)

-- Custom Types
DO $$ BEGIN CREATE TYPE public.user_role AS ENUM ('mother', 'obstetrician', 'pediatrician', 'admin'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE public.link_status AS ENUM ('pending', 'accepted', 'rejected', 'revoked'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN CREATE TYPE public.pregnancy_status AS ENUM ('active', 'completed', 'terminated'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE public.prescription_status AS ENUM ('activa', 'expirada', 'cancelada', 'dispensada'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE public.saas_plan AS ENUM ('madre_basico', 'madre_premium', 'doc_basico', 'doc_pro', 'clinica', 'hospital', 'enterprise'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE public.doc_status AS ENUM ('pending', 'under_review', 'approved', 'rejected', 'suspended'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE public.attachment_type AS ENUM ('image', 'pdf', 'audio'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
-- 9. PEDIATRIC VISITS Table
CREATE TABLE IF NOT EXISTS public.pediatric_visits (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- 17. DEVELOPMENT MILESTONES Table
...
-- POLÍTICAS DE RLS (Muestra representativa, el script completo tiene todas)
...
-- Políticas para mothers
DROP POLICY IF EXISTS "Mothers can view their own data." ON public.mothers;
CREATE POLICY "Mothers can view their own data." ON public.mothers FOR SELECT USING (auth.uid() = id);
DROP POLICY IF EXISTS "Mothers can insert their own data." ON public.mothers;
CREATE POLICY "Mothers can insert their own data." ON public.mothers FOR INSERT WITH CHECK (auth.uid() = id);
DROP POLICY IF EXISTS "Mothers can update their own data." ON public.mothers;
...
DROP POLICY IF EXISTS "Public profiles read professionals details" ON public.professionals;
CREATE POLICY "Public profiles read professionals details" ON public.professionals FOR SELECT USING (true);
DROP POLICY IF EXISTS "Professionals can update their own professional detail" ON public.professionals;
...
DROP POLICY IF EXISTS "Mothers can view their own pregnancies." ON public.pregnancies;
CREATE POLICY "Mothers can view their own pregnancies." ON public.pregnancies FOR SELECT USING (auth.uid() = mother_id);
DROP POLICY IF EXISTS "Mothers can insert their own pregnancies." ON public.pregnancies;
CREATE POLICY "Mothers can insert their own pregnancies." ON public.pregnancies FOR INSERT WITH CHECK (auth.uid() = mother_id);
DROP POLICY IF EXISTS "Mothers can update their own pregnancies." ON public.pregnancies;
...
DROP POLICY IF EXISTS "Mothers can view their own babies." ON public.babies;
CREATE POLICY "Mothers can view their own babies." ON public.babies FOR SELECT USING (auth.uid() = mother_id);
DROP POLICY IF EXISTS "Mothers can insert their own babies." ON public.babies;
CREATE POLICY "Mothers can insert their own babies." ON public.babies FOR INSERT WITH CHECK (auth.uid() = mother_id);
DROP POLICY IF EXISTS "Mothers can update their own babies." ON public.babies;
CREATE POLICY "Mothers can update their own babies." ON public.babies FOR UPDATE USING (auth.uid() = mother_id);

-- Políticas para doctor_patient_links
DROP POLICY IF EXISTS "Users can read links they are involved in" ON public.doctor_patient_links;
...
DROP POLICY IF EXISTS "Mothers can create link requests" ON public.doctor_patient_links;
CREATE POLICY "Mothers can create link requests" ON public.doctor_patient_links FOR INSERT WITH CHECK (auth.uid() = mother_id);
DROP POLICY IF EXISTS "Professionals/Mothers can update statuses" ON public.doctor_patient_links;
...
DROP POLICY IF EXISTS "Mothers can create appointments." ON public.appointments;
...
-- Políticas para prenatal_visits
DROP POLICY IF EXISTS "Mothers can view their own prenatal visits." ON public.prenatal_visits;
...
-- Políticas para pediatric_visits
DROP POLICY IF EXISTS "Mothers can view their own pediatric visits." ON public.pediatric_visits;
...
CREATE POLICY "Users can view their own lab results." ON public.lab_results FOR SELECT USING (auth.uid() = mother_id);

-- Políticas para ultrasound_results
DROP POLICY IF EXISTS "Mothers can view their own ultrasound results." ON public.ultrasound_results;
...
DROP POLICY IF EXISTS "Mothers can manage their own symptoms." ON public.symptoms;
...
DROP POLICY IF EXISTS "Users can manage their own vital signs." ON public.vital_signs;
...
-- Políticas para baby_vaccines
DROP POLICY IF EXISTS "Mothers can view their own baby vaccines." ON public.baby_vaccines;
...
DROP POLICY IF EXISTS "Mothers can view their own baby growth records." ON public.growth_records;
...
-- Políticas para development_milestones
DROP POLICY IF EXISTS "Mothers can manage their own baby development milestones." ON public.development_milestones;
...
DROP POLICY IF EXISTS "Professionals can manage their own professional documents" ON public.professional_documents;
...
-- Políticas para mother_documents
DROP POLICY IF EXISTS "Mothers can manage their own verification documents" ON public.mother_documents;
...
DROP POLICY IF EXISTS "Professionals can manage prescriptions they write" ON public.prescriptions;
CREATE POLICY "Professionals can manage prescriptions they write" ON public.prescriptions FOR ALL USING (auth.uid() = doctor_id);
DROP POLICY IF EXISTS "Mothers can view prescriptions issued for them" ON public.prescriptions;
...
DROP POLICY IF EXISTS "Users can read items if they have access to parent prescription" ON public.prescription_items;
...
DROP POLICY IF EXISTS "Mothers can view their own clinical history." ON public.clinical_histories;
CREATE POLICY "Mothers can view their own clinical history." ON public.clinical_histories FOR SELECT USING (auth.uid() = id);
DROP POLICY IF EXISTS "Mothers can update their own clinical history." ON public.clinical_histories;
...
-- Políticas para hipaa_consent_logs
DROP POLICY IF EXISTS "Users can view their own HIPAA consent logs" ON public.hipaa_consent_logs;
CREATE POLICY "Users can view their own HIPAA consent logs" ON public.hipaa_consent_logs FOR SELECT USING (auth.uid() = user_id);

-- Políticas para privacy_requests
DROP POLICY IF EXISTS "Users can view and manage their own privacy requests" ON public.privacy_requests;
...
-- Políticas para cms_articles
DROP POLICY IF EXISTS "Anyone can view published cms articles" ON public.cms_articles;
...
-- =============================================================================
-- COMPLEMENTOS (añadidos por la migración 2026-06)
-- =============================================================================

-- Columnas que pueden faltar en instalaciones previas
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS suspension_reason TEXT;

-- Función SECURITY DEFINER para comprobar rol admin SIN recursión de RLS
...
-- Políticas de administrador (gestión total de perfiles y datos clínicos base)
...
DROP POLICY IF EXISTS "Admins can view all mothers" ON public.mothers;
CREATE POLICY "Admins can view all mothers" ON public.mothers FOR SELECT USING (public.is_admin());

DROP POLICY IF EXISTS "Admins can view all pregnancies" ON public.pregnancies;
CREATE POLICY "Admins can view all pregnancies" ON public.pregnancies FOR SELECT USING (public.is_admin());

DROP POLICY IF EXISTS "Admins can view all babies" ON public.babies;
CREATE POLICY "Admins can view all babies" ON public.babies FOR SELECT USING (public.is_admin());

-- Los médicos vinculados pueden ver los datos de sus pacientes
DROP POLICY IF EXISTS "Linked doctors can view mother data" ON public.mothers;
CREATE POLICY "Linked doctors can view mother data" ON public.mothers FOR SELECT USING (
  EXISTS (
