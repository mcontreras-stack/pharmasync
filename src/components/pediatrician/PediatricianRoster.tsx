...
export default function PediatricianRoster({
  babyProfiles,
  selectedBabyId,
  onSelectBaby,
  searchQuery,
  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-4 space-y-4 text-left select-none">
      <h3 className="font-bold text-gray-800 flex items-center gap-2 px-2 text-sm">
        ...
        Niños Pacientes ({babyProfiles.length})
      </h3>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-gray-50 border border-gray-200 rounded-xl py-2 pl-10 pr-4 text-xs focus:outline-emerald-500 font-semibold text-slate-800"
        />
      </div>
        ...
        {babyProfiles.length > 0 ? (
          babyProfiles.map(p => {
            ...
            return (
              <div
                key={p.baby.id}
                onClick={() => onSelectBaby(p.baby.id)}
                className={`p-3 rounded-2xl border transition-all duration-200 cursor-pointer flex items-center justify-between ${
                  isActive 
                    ...
                    : 'bg-gray-50/50 border-gray-200 hover:border-emerald-200 hover:bg-gray-50'
                }`}
              >
                <div className="min-w-0">
                  <h4 className="text-xs font-bold truncate">{p.baby.name}</h4>
                  <p className={`text-[9px] ${isActive ? 'text-emerald-100' : 'text-gray-400'} mt-0.5 font-semibold truncate`}>
